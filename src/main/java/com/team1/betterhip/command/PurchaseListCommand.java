...
public class PurchaseListCommand implements MyPageCommand {
	...
	public void execute3(HttpServletRequest request, HttpSession session, SqlSession sqlSession, Model model) {
		...
		//주문 상태별 주문 수 가져오는 다오 실행  
		
		PurchaseDao dao1 = sqlSession.getMapper(PurchaseDao.class);
		model.addAttribute("purchase_status_count_3", dao1.purchase_status_count_3(USER_ID));
		
		
		PurchaseDao dao2 = sqlSession.getMapper(PurchaseDao.class);
		model.addAttribute("purchase_status_count_101", dao2.purchase_status_count_101(USER_ID));
		
		
		PurchaseDao dao3 = sqlSession.getMapper(PurchaseDao.class);
		model.addAttribute("purchase_status_count_103", dao3.purchase_status_count_103(USER_ID));
		
		//주문 조회 리스트 다오 실행 
		
		PurchaseDao dao4 = sqlSession.getMapper(PurchaseDao.class); 
		ArrayList<PurchaseListDto> dtos = dao4.purchaseList(USER_ID); 
		
			//데이터 정제 
			for(int i=0; i<dtos.size(); i++) {
				
				//구매금액 decimal format 설정 
